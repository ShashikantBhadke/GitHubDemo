...
final class RepoListModelClass: ObservableObject {

    required init(strSearch: String) {
        self.strFullPath = strSearch
        if !strSearch.isEmpty {
            getData()
        } else {
            state = .fetched(.failure("No proper path"))
        }
    }
    
    var strFullPath = ""
    var state: LoadableState<[RepoListModelElement]> = .loading
    @Published var gitRepos = [RepoListModelElement]()

    // MARK:- WebService
    func getData() {
        WebService.call(withAPI: strFullPath, withDecodabel: [RepoListModelElement].self) { (result) in
            switch result {
...
// MARK: - RepoListModelElement
struct RepoListModelElement: Codable, Identifiable {
    var id: Int
    var nodeID, name, fullName: String?
    var repoListModelPrivate: Bool?
    var owner: Owner?
    var htmlURL: String?
    var repoListModelDescription: String?
    var fork: Bool?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case repoListModelPrivate = "private"
        case owner
        case htmlURL = "html_url"
        case repoListModelDescription = "description"
        case fork, url
